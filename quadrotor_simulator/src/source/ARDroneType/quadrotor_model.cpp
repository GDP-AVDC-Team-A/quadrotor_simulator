#include "ARDroneType/quadrotor_model.h"
...
//Parrot Modelo 1
void QuadrotorModel::processModel(Vector* Statek1, Vector* Statek, Vector* Inputs)
{
  // ************** State ************** 
  // Xestate order
...
void QuadrotorModel::observationModel(Vector* Output, Vector* Statek)
{
	//  Medidas:
	//  01 pitch: C(1,1)*X1
	//  02 roll : C(2,2)*X2
	...
	float Paux_k = Statek->getValueData(1);
	float Raux_k = Statek->getValueData(2);
    //[unused_variable] float dYaux_k = Statek->getValueData(3);
	float dYp_k = Statek->getValueData(4);
	float Y_k = Statek->getValueData(5);
    //[unused_variable] float dZaux_k = Statek->getValueData(6);
	float dZp_k = Statek->getValueData(7);
	float Z_k = Statek->getValueData(8);
	float x_k = Statek->getValueData(9);
	float y_k = Statek->getValueData(10);
...
void QuadrotorModel::jacobiansProcessModel(Matrix* MatJacFx, Matrix* MatJacFu, Vector* Statek, Vector* Inputs)
{
	...
	float Raux_k = Statek->getValueData(2);
    //[unused_variable] float dYaux_k = Statek->getValueData(3);
    //[unused_variable] float dYp_k = Statek->getValueData(4);
	float Y_k = Statek->getValueData(5);
    //[unused_variable] float dZaux_k = Statek->getValueData(6);
    //[unused_variable] float dZp_k = Statek->getValueData(7);
    //[unused_variable] float Z_k = Statek->getValueData(8);
    //[unused_variable] float x_k = Statek->getValueData(9);
    //[unused_variable] float y_k = Statek->getValueData(10);
	float vx_k = Statek->getValueData(11);
	float vy_k = Statek->getValueData(12);
...
void QuadrotorModel::jacobiansObservationModel(Matrix* MatJacHx, Vector* Statek)
{
	//  Medidas:
	//  01 pitch: C(1,1)*X1
	//  02 roll : C(2,2)*X2
    ...
    //[unused_variable] float Paux_k = Statek->getValueData(1);
    //[unused_variable] float Raux_k = Statek->getValueData(2);
    //[unused_variable] float dYaux_k = Statek->getValueData(3);
    //[unused_variable] float dYp_k = Statek->getValueData(4);
	float Y_k = Statek->getValueData(5);
    //[unused_variable] float dZaux_k = Statek->getValueData(6);
...
void QuadrotorModel::getObservation(double &x_out, double &y_out, double &z_out, double &yaw_out, double &pitch_out, double &roll_out, double &vx_out, double &vy_out, double &vz_out, double &dyaw_out, double &vxm_out, double &vym_out)
{
    observationModel( &Observation, &Statek);

    pitch_out = Observation.getValueData(1);
    roll_out  = Observation.getValueData(2);
    dyaw_out  = Observation.getValueData(3);
    yaw_out   = Observation.getValueData(4);
    yaw_out   = atan2( sin(yaw_out), cos(yaw_out));
    Statek.setValueData(         yaw_out,5);
    vz_out    = Observation.getValueData(5);
    z_out     = Observation.getValueData(6);
...
void QuadrotorModel::runSimulation()
{
...
void QuadrotorModel::start(double x_in, double y_in, double z_in, double yaw_in, double pitch_in, double roll_in, double vx_in, double vy_in, double vz_in, double dyaw_in)
{
    timer.restart(false);

    Statek.setValueData( pitch_in/58.227, 1);
    Statek.setValueData(  roll_in/38.575, 2);
    // Statek.setValueData(                , 3);
    Statek.setValueData(  dyaw_in/67.213, 4);
    Statek.setValueData(          yaw_in, 5);
    // Statek.setValueData(                , 6);
    Statek.setValueData( vz_in/(-14.189), 7);
    Statek.setValueData(            z_in, 8);
